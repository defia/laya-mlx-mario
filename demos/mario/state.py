    ...
    def navigation_features(self) -> dict[str, Any]:
        ...
        for column in range(mario_col + 1, len(rows[0])):
            ...
            if ground_row is not None:
                ...
                # support at ANY depth: a step-down (floor one row lower) is
                # walkable, not a gap — checking only ground_row read every
                # elevation change as a phantom multi-tile gap
                supported = any(
                    rows[r][column] == "#" for r in range(ground_row, len(rows))
                )
...
class MarioStateParser:
    ...
    def parse(
        ...
        *,
        ...
        # per-parse deltas must be normalized to px/frame like enemy velocity
        frame_step = max(1, frames_elapsed)
        dx = 0 if self._last_x is None else round((x - self._last_x) / frame_step)
        dy = 0 if self._last_y is None else round((y - self._last_y) / frame_step)
        ...
        enemies = self._extract_enemies(info, ram, x, screen_y, frames_elapsed)
        # per-parse counters must track real frames: in the headless loop one
        # parse spans frames_per_decision emulator frames, not one
    ...
    def _extract_enemies(
        ...
        for slot in range(5):
            ...
            # normalize to px/frame: one parse may span many emulator frames
            relative_velocity_x = (
                0
                if previous_dx is None
                else round((dx - previous_dx) / max(1, frames_elapsed))
            )
    ...
    def _extract_local_grid(
        ...
        # The two tile pages hold the world relative to MARIO's page/screen
        # position (RAM 0x6D page counter, 0x86 screen x), like the enemy
        # slots. Mapping from world x misreads the page once a sample crosses
        # a 256px boundary — measured: an 8-tile phantom "gap" after the
        # first pipe, and ground vanishing past x≈550.
        mario_page = self._ram_byte(ram, 0x006D, mario_x // 256)
        mario_screen_x = self._ram_byte(ram, 0x0086, mario_x % 256)
        ...
        for row, dy_tiles in enumerate(y_offsets):
            for col, dx_tiles in enumerate(x_offsets):
                ...
                screen_x = mario_screen_x + box_x
                page = (mario_page + screen_x // 256) % 2
                sub_x = (screen_x % 256) // 16
